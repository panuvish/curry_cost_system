...
function renderIngredients() {
  ...
  const list = ingredients
    .filter(x =>
      ...
      String(x.code || "").toLowerCase().includes(keyword)
    )
    .sort((a,b) => String(a.code || "").localeCompare(String(b.code || "")));

  $("ingredientTable").innerHTML = list.map(item => `
    <tr>
  ...
  $("rawCount").textContent = ingredients.filter(x => x.category === "DM").length;
  $("packCount").textContent = ingredients.filter(x => x.category === "PACKAGING").length;
...
$("closeModal").addEventListener("click", () => $("ingredientModal").classList.add("hidden"));
$("cancelBtn").addEventListener("click", () => $("ingredientModal").classList.add("hidden"));
...
$("ingredientForm").addEventListener("submit", async (e) => {
  ...
  try {
    if (editingId) {
      ...
      await addDoc(ingredientCol, { ...data, stockQty: 0, createdAt: serverTimestamp() });
...
async function seedData() {
  ...
  const ingredients = [
    ...
    ["PK003","ซอง 500 กรัม ชั้นใน","PACKAGING",100,"ใบ",70,0.70,"ใบ"],
    ["PK004","ซอง 500 กรัม ชั้นนอก","PACKAGING",100,"ใบ",150,1.50,"ใบ"],
    ["PK005","ค่าสกรีน","PACKAGING",1,"ซอง",2,2,"ซอง"]
  ];

  ingredients.forEach(([code,name,category,purchaseQty,purchaseUnit,purchasePrice,costPerUnit,costUnit]) => {
    const ref = doc(ingredientCol);
    batch.set(ref, {
      code,name,category,purchaseQty,purchaseUnit,purchasePrice,costPerUnit,costUnit,
      stockQty: 0, createdAt: serverTimestamp(), updatedAt: serverTimestamp()
    });
  });
  ...
  labor.forEach(([code,name,rate]) => {
    ...
    batch.set(ref, { code, name, rate, unit: "บาท/ชั่วโมง", createdAt: serverTimestamp() });
  });
...
onSnapshot(laborCol, snapshot => {
  $("laborTable").innerHTML = snapshot.docs.map(d => {
    const x = d.data();
...
onSnapshot(overheadCol, snapshot => {
  $("overheadTable").innerHTML = snapshot.docs.map(d => {
    const x = d.data();
